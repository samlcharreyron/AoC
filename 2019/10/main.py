...
if __name__ == '__main__':
    ...
    for i in indices:
        ...
        rays_n = rays / np.tile(lengths, (2,1)).T
        rays_n = rays_n.round(4)
        num_ast[i] = len(np.unique(rays_n, axis=0))
    print(max(num_ast))
    ...
    rays_n = rays / np.tile(lengths, (2,1)).T
    rays_n = rays_n.round(4)
    angles = np.mod(np.arctan2(rays_n[:,1], rays_n[:,0]) + np.pi/2 + 2*np.pi, 2*np.pi)
    ...
    while sum(len(v) for v in radar.values()):
        ...
        for angle in sorted(radar.keys()):
            if radar[angle]:
                ...
                if n == 200:
                    print('removing {} with code {}'.format(p, 100*p[0]+p[1]))
        i += 1
